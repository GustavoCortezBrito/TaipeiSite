...
// Fallback com fotos locais — usado enquanto não há token da API
  ...
  return (
    ...
    >
      <Image
        ...
        unoptimized={post.image.startsWith("http")} // URLs externas (API) não passam pelo optimizer
      />
...
export default function InstagramFeedAPI() {
    ...
    const load = async () => {
      try {
        ...
        if (res.ok && data.posts?.length > 0) {
          ...
          // API sem token ou erro → usa fotos locais silenciosamente
  ...
  return (
    <section className="py-24 px-4 bg-gradient-to-b from-white to-taipei-cream">
      <div className="container mx-auto max-w-7xl">
        ...
        {/* Loading spinner central (se demorar muito) */}
        {loading && (
          <div className="flex justify-center py-8">
            <Loader2 className="text-taipei-red animate-spin" size={32} />
          </div>
        )}
